...
vi.mock("@uiw/react-codemirror", () => ({
  default: ({
    ...
    <textarea
      ...
      data-skill-highlight={
        ...
        extensions.includes(sourceSkillHighlightMock.extension)
          ? "true"
          : "false"
      }
      data-skill-completion={
        Array.isArray(extensions) &&
        extensions.includes(sourceSkillCompletionMock.extension)
          ? "true"
          : "false"
      }
    />
  ...
  StateEffect: {
    define: () => ({ of: (value: unknown) => ({ value }), is: () => false }),
  },
...
function createMockTiptapEditor(
  markdown: string,
  segments: MockTiptapTextSegment[],
) {
  ...
  const replaceRange = (from: number, to: number, text: string) => {
    const startIndex = mutableSegments.findIndex(
      (segment) =>
        from >= segment.from && from <= segment.from + segment.text.length,
    );
    const endIndex = mutableSegments.findIndex(
      (segment) =>
        to >= segment.from && to <= segment.from + segment.text.length,
    );
  ...
  editor.storage = {
    markdown: {
      manager: {
        serialize: vi.fn(
          (slice: any) => slice?.__markdownSlice ?? editor.markdown,
        ),
      },
    },
  };
...
describe("App recent documents", () => {
  ...
  beforeEach(() => {
    ...
    activeDocumentDiskSourceMock.mockReset();
    activeDocumentDiskSourceMock.mockRejectedValue(
  ...
  it("exposes aria-keyshortcuts on the EditorArea empty-state action buttons", async () => {
    ...
    expect(newFileButton).toHaveAttribute(
      "aria-keyshortcuts",
      "Meta+N Control+N",
    );
    expect(openFileButton).toHaveAttribute(
      "aria-keyshortcuts",
      "Meta+O Control+O",
    );
    expect(openWorkspaceButton).toHaveAttribute(
  ...
  it("surfaces open failures without leaving an unhandled runtime error", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(
          screen.queryByRole("status", { name: /working/i }),
  ...
  it("opens a validated AI result in a transient review tab before applying it", async () => {
    ...
    expect(await screen.findByText("Improve the heading.")).toBeInTheDocument();
      ...
      .find(
        (tab) =>
          !tab
            ...
            ?.startsWith("__markdowner_ai_review__"),
      );
  ...
  it("confirms cancellation before closing a running AI Review tab", async () => {
    ...
    expect(
      await screen.findByText(/AI request in progress/i),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(aiCancelMock).toHaveBeenCalledWith(expect.any(String)),
    );
    expect(messageMock).toHaveBeenCalled();
    expect(
      screen.queryByText(/AI request in progress/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("applies a WYSIWYG drag-selection prompt in one editor transaction", async () => {
    ...
    fireEvent.change(await screen.findByLabelText("Prompt for selected text"), {
  ...
  it("smoke-tests empty-state controls without runtime errors", async () => {
    ...
    try {
      ...
      const quickOpenDialog = await screen.findByRole("dialog", {
        name: /quick open/i,
      });
      ...
      fireEvent.click(
        screen.getByRole("button", { name: /^settings \(cmd\+,\)$/i }),
      );
  ...
  it("smoke-tests active document menus and commands without runtime errors", async () => {
    ...
    setThemeMock.mockImplementation(
      async (themeKind: AppSnapshot["theme"]["kind"]) => ({
        ...activeSnapshot,
        theme: {
          kind: themeKind,
          stylesheet: null,
    ...
    try {
      ...
      fireEvent.click(
        within(menu).getByRole("menuitem", { name: /^import css…$/i }),
      );
      await waitFor(() => expect(openDialogMock).toHaveBeenCalled());

      menu = await openAppMenu();
      fireEvent.click(
        within(menu).getByRole("menuitemradio", { name: /^split view$/i }),
      );
      await waitFor(() =>
      ...
      menu = await openAppMenu();
      fireEvent.click(
        within(menu).getByRole("menuitemradio", { name: /^light theme$/i }),
      );
      await waitFor(() =>
      ...
      fireEvent.click(
        screen.getByRole("button", { name: /^outline \(cmd\+shift\+d\)$/i }),
      );
      const agendaHeading = await screen.findByRole("button", {
        name: /^agenda$/i,
      });
  ...
  it("opens an Outline sidebar view with document headings from the active draft", async () => {
    ...
    expect(
      screen
        .getAllByText(/^outline$/i)
        .some((node) => node.classList.contains("explorer-section-header")),
    ).toBe(true);
    expect(
  ...
  it("renders Search and Outline panels with the Explorer sidebar density", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /search \(cmd\+shift\+f\)/i }),
    );
    ...
    expect(within(searchPanel).getByText(/^search$/i)).toHaveClass(
      "explorer-section-header",
    );
    expect(within(searchPanel).getByTestId("sidebar-search-input")).toHaveClass(
      "h-7",
    );
    ...
    expect(
      within(outline).getByRole("button", { name: /^agenda$/i }),
    ).toHaveClass("explorer-tree-row");
  });
  ...
  it("focuses and navigates Outline rows with Cmd+0, arrows, and Enter", async () => {
    ...
    const agenda = await within(outline).findByRole("button", {
      name: /^agenda$/i,
    });
    const decisions = within(outline).getByRole("button", {
      name: /^decisions$/i,
    });
  ...
  it("exposes Split View source and preview panes as named landmark regions", async () => {
    ...
    expect(sourceRegion).toHaveAttribute(
      "data-testid",
      "editor-surface-source",
    );
    expect(previewRegion).toHaveAttribute(
      "data-testid",
  ...
  it("syncs Split View source and preview scrolling proportionally", async () => {
    bootstrapMock.mockResolvedValue(
      baseSnapshot({
        activeDocumentName: "notes.md",
        activeDocumentPath: "/tmp/project/notes.md",
        activeDocumentSource: Array.from(
          { length: 80 },
  ...
  it("renders workspace documents in a nested folder tree", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /draft\.md/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /api\.md/i }),
    ).toBeInTheDocument();
  });
  ...
  it("renders a VS Code-like Explorer sidebar with open editors and workspace sections", async () => {
    ...
    expect(
      within(explorer).getByTestId("explorer-open-editors"),
    ).toHaveTextContent("draft.md");
    expect(
      within(explorer).getByRole("button", { name: /new file/i }),
    ).toBeInTheDocument();
    expect(
      within(explorer).getByRole("button", { name: /open workspace/i }),
    ).toBeInTheDocument();
    expect(
      within(explorer).getByRole("button", { name: /collapse all/i }),
    ).toBeInTheDocument();
    const workspaceTree = within(explorer).getByTestId(
      "explorer-workspace-tree",
    );
    expect(
      within(workspaceTree).getByRole("button", { name: /guides/i }),
    ).toHaveClass("explorer-tree-row");
    expect(
      within(workspaceTree).getByRole("button", { name: /draft\.md/i }),
    ).toHaveClass("explorer-tree-row");
  ...
  it("moves Explorer row focus with arrows after Cmd+0 and opens the focused file with Cmd+Down", async () => {
    ...
    const guides = await within(explorer).findByRole("button", {
      name: /^guides$/i,
    });
    ...
    const reference = within(explorer).getByRole("button", {
      name: /^reference$/i,
    });
    await waitFor(() => {
  ...
  it("collapses and re-expands workspace folders from the sidebar", async () => {
    ...
    const guidesFolderButton = await screen.findByRole("button", {
      name: "guides",
    });
    ...
    expect(
      screen.queryByRole("button", { name: /draft\.md/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /api\.md/i }),
    ).not.toBeInTheDocument();
  ...
  it("filters workspace files while keeping matching folder ancestry visible", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole("button", { name: "reference" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /api\.md/i }),
      ).toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("button", { name: /readme\.md/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders friendly theme labels in the status bar instead of the raw enum", async () => {
    ...
    expect(
      within(statusBar as HTMLElement).queryByText(/BuiltInDark/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("exposes descriptive tooltips on the StatusBar mode, theme, and save-status spans", async () => {
    ...
    expect(scope.getByText(/^Editor$/)).toHaveAttribute(
      "title",
      "Active editor mode",
    );
  ...
  it("surfaces the active document name and path in the status bar", async () => {
    ...
    await waitFor(() => {
      expect(documentLabel).toHaveAttribute(
        "title",
        "/tmp/project/meeting-notes.md",
      );
    });
  ...
  it("exposes a System theme toggle that persists themeFollowSystem through settings", async () => {
    ...
    setThemeMock.mockImplementation(
      async (kind: "BuiltInLight" | "BuiltInDark" | "CustomCss") =>
        baseSnapshot({
          activeDocumentName: "meeting-notes.md",
          activeDocumentPath: "/tmp/project/meeting-notes.md",
          activeDocumentSource: "# Meeting notes",
    ...
    let menu = await openAppMenu();
    let lightToggle = within(menu).getByRole("menuitemradio", {
      name: /light theme/i,
    });
    const darkToggle = within(menu).getByRole("menuitemradio", {
      name: /dark theme/i,
    });
    ...
    menu = await openAppMenu();
    lightToggle = within(menu).getByRole("menuitemradio", {
      name: /light theme/i,
    });
  ...
  it("applies the current OS theme when Follow System Theme is enabled from Settings", async () => {
    ...
    const themeToggleGroup = within(dialog).getByTestId(
  ...
  it("saves the active document to a new path from the shell", async () => {
    ...
    saveDialogMock.mockResolvedValue(
    ...
    const menu = await openAppMenu();
    const saveAsButton = within(menu).getByRole("menuitem", {
      name: /^save as…$/i,
    });
  ...
  it("previews and styles the active document before exporting to a PDF path", async () => {
    ...
    const menu = await openAppMenu();
    fireEvent.click(
      within(menu).getByRole("menuitem", { name: /^export as pdf…$/i }),
    );
    ...
    expect(
      (screen.getByLabelText("Code block theme") as HTMLSelectElement)
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTestId("export-preview-surface").parentElement,
      ).toHaveClass("hidden");
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("tab", { name: /Export Preview/i }),
      ).not.toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /meeting-notes\.md/i }),
      ).toHaveAttribute("aria-selected", "true");
    });

    const reopenedMenu = await openAppMenu();
    fireEvent.click(
      within(reopenedMenu).getByRole("menuitem", { name: /^export as pdf…$/i }),
    );
    expect(await screen.findByLabelText("Theme")).toHaveValue("custom");
    expect(await screen.findByLabelText("Body size")).toHaveValue("13");
    expect(screen.getByLabelText("Background color")).toHaveValue("#18181b");
    expect(screen.getByLabelText("Code block theme")).toHaveValue(
      "flexoki-dark",
    );
  ...
  it("exports the current document as one continuous WebP image using returned paths", async () => {
    ...
    const menu = await openAppMenu();
    fireEvent.click(
      within(menu).getByRole("menuitem", { name: /^export as image…$/i }),
    );

    expect(
      await screen.findByRole("combobox", { name: "Image format" }),
    ).toHaveValue("png");
    ...
    expect(
      JSON.parse(
        window.localStorage.getItem("markdowner.imageExportOptions.v1") ?? "{}",
      ),
  ...
  it("keeps Image Export Preview open and shows the native renderer error", async () => {
    ...
    exportImageFileMock.mockRejectedValue(
      new Error(
        "Image output '/tmp/project/exports/meeting-notes-001.png' already exists",
      ),
    );
    ...
    expect(
      screen.getByText(/meeting-notes-001\.png.*already exists/i),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("tab", { name: /Export Preview/i }),
    ).toHaveAttribute("aria-selected", "true");
  });
  ...
  it("keeps the Export Preview tab open when the native save dialog is cancelled", async () => {
    ...
    const menu = await openAppMenu();
    fireEvent.click(
      within(menu).getByRole("menuitem", { name: /^export as html…$/i }),
    );
    const exportTab = await screen.findByRole("tab", {
      name: /Export Preview/i,
    });
    ...
    expect(exportTab).toHaveAttribute("aria-selected", "true");
    expect(
      screen.getByRole("heading", { name: "Export Preview" }),
    ).toBeInTheDocument();
  });
  ...
  it("closes Export Preview with Cancel and restores the previous document tab", async () => {
    ...
    expect(
      await screen.findByRole("tab", { name: /Export Preview/i }),
    ).toHaveAttribute("aria-selected", "true");
  ...
  it("flushes the live WYSIWYG draft before exporting HTML so inserted images are embedded", async () => {
    const editor = createMockTiptapEditor("# Meeting notes", [
      { text: "# Meeting notes", from: 1 },
    ]);
    tiptapMockState.editor = editor;
    bootstrapMock.mockResolvedValue(
      baseSnapshot({
    ...
    readImagesBase64Mock.mockImplementation(
      async (sources: readonly string[]) =>
        sources.map((source) => ({
          source,
          dataUri: `data:image/png;base64,EMBED(${source})`,
        })),
    );
    ...
    await waitFor(() => {
      expect(readImagesBase64Mock).toHaveBeenCalledWith([
        "/tmp/project/assets/shot.png",
      ]);
      expect(exportTextFileMock).toHaveBeenCalledWith(
        "/tmp/project/exports/meeting-notes.html",
        expect.stringContaining(
          'src="data:image/png;base64,EMBED(/tmp/project/assets/shot.png)"',
        ),
      );
    });
  });
  ...
  it("exports every workspace markdown file to PDFs under the project exports folder", async () => {
    ...
    fireEvent.click(
      within(menu).getByRole("menuitem", {
        name: /^export all markdown as pdfs…$/i,
      }),
    );

    expect(exportPdfFilesMock).not.toHaveBeenCalled();
    const exportButton = await screen.findByRole("button", {
      name: "Export 2 PDF files",
    });
    fireEvent.change(screen.getByLabelText("Size"), {
      target: { value: "Custom" },
    });
  ...
  it("exports every workspace Markdown file to HTML under the project exports folder", async () => {
    ...
    fireEvent.click(
      within(menu).getByRole("menuitem", {
        name: /^export all markdown as html…$/i,
      }),
    );

    expect(exportTextFilesMock).not.toHaveBeenCalled();
    const exportButton = await screen.findByRole("button", {
      name: "Export 2 HTML files",
    });
    await waitFor(() => expect(exportButton).toBeEnabled());
    fireEvent.click(exportButton);
  ...
  it("keeps a switched tab active when an earlier Save As resolves later", async () => {
    ...
    const menu = await openAppMenu();
    fireEvent.click(
      within(menu).getByRole("menuitem", { name: /^save as…$/i }),
    );

    await waitFor(() => {
      expect(saveActiveDocumentAsMock).toHaveBeenCalledWith(
        "/tmp/project/alpha-copy.md",
      );
  ...
  it("keeps a switched tab active when an earlier Save resolves later", async () => {
    ...
    await waitFor(() => {
      expect(sourceEditor).toHaveValue("# Alpha");
      expect(within(statusBar).getAllByTitle(alphaPath).length).toBeGreaterThan(
        0,
      );
    });
    ...
    await waitFor(() => {
      expect(sourceEditor).toHaveValue("# Beta");
      expect(within(statusBar).getAllByTitle(betaPath).length).toBeGreaterThan(
        0,
      );
  ...
  it("does not show a stale external-change warning after switching tabs during Save", async () => {
    ...
    expect(
      screen.queryByText(
        /Could not save 'alpha\.md' because it changed on disk\./i,
      ),
    ).not.toBeInTheDocument();
    expect(screen.getByRole("tab", { name: /beta\.md/i })).toHaveAttribute(
      "aria-selected",
  ...
  it("exposes keyboard-shortcut tooltips on app menu file actions", async () => {
    ...
    expect(importCssButton).toHaveAttribute(
      "title",
  ...
  it("exposes aria-keyshortcuts on app menu shortcut actions and mode items", async () => {
    ...
    const editorToggle = within(menu).getByRole("menuitemradio", {
      name: "Editor",
    });
    const wysiwygToggle = within(menu).getByRole("menuitemradio", {
      name: "WYSIWYG",
    });
    const splitToggle = within(menu).getByRole("menuitemradio", {
      name: "Split View",
    });
  ...
  it("exposes aria-keyshortcuts on the ActivityBar Explorer/Search/Settings buttons", async () => {
    ...
    expect(explorerButton).toHaveAttribute(
      "aria-keyshortcuts",
  ...
  it("exposes consistent tooltips on app menu theme items", async () => {
    ...
    const menu = await openAppMenu();
    const lightToggle = within(menu).getByRole("menuitemradio", {
      name: /light theme/i,
    });
    const darkToggle = within(menu).getByRole("menuitemradio", {
      name: /dark theme/i,
  ...
  it("starts a fresh window URL with an untitled tab instead of restoring previous tabs", async () => {
    ...
    await screen.findAllByText(/^Untitled\.md/);
    expect(newDocumentMock).toHaveBeenCalled();
    expect(openDocumentMock).not.toHaveBeenCalledWith(
      "/tmp/project/meeting-notes.md",
    );
  });
  ...
  it("stacks a new Untitled tab on every Cmd+N instead of reusing one", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByRole("tab", { name: /untitled\.md/i }),
      ).toHaveLength(1);
    });
  ...
  it("closes the only tab on Cmd+W without prompting when the active tab has no local edits", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByRole("tablist", { name: /open documents/i }),
      ).toBeNull();
    });
    expect(
  ...
  it("lets the user reload from disk when external changes are detected", async () => {
    ...
    fireEvent.click(
      within(secondMenu).getByRole("menuitem", { name: /^save$/i }),
    );
  ...
  it("polls a clean document for disk changes while the window stays focused", async () => {
    ...
    try {
      ...
      const sourceEditor = await screen.findByLabelText("Source editor");
      await waitFor(() => {
        expect(intervalCallbacks.some(({ timeout }) => timeout === 1000)).toBe(
          true,
        );
      });
      const refresh = intervalCallbacks.find(
  ...
  it("does not mistake the backend cache for a clean tab after switching back", async () => {
    ...
    replaceActiveDocumentSourceMock.mockImplementation((source: string) => {
      ...
      return Promise.resolve(
        baseSnapshot({
          activeDocumentName:
            backendActivePath === alphaPath ? "alpha.md" : "beta.md",
          activeDocumentPath: backendActivePath,
          activeDocumentSource: source,
          activeDocumentDirty: true,
          mode: "Editor",
  ...
  it("preserves a dirty switched tab when external-change verification fails", async () => {
      ...
      ({ path }: { path: string }) =>
        Promise.resolve(
          baseSnapshot({
            ...
            activeDocumentPath: path,
            activeDocumentSource:
              path === alphaPath ? "# Alpha disk" : "# Beta disk",
            mode: "Editor",
          }),
        ),
    );
    ...
    hasActiveDocumentExternalChangesMock.mockRejectedValueOnce(
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          /Could not verify external changes.*permission denied/i,
        ),
      ).toBeInTheDocument();
      expect(screen.getByRole("tab", { name: /beta\.md/i })).toHaveAttribute(
        "aria-selected",
    ...
    openDocumentMock.mockImplementation((path: string) =>
      Promise.resolve(
        baseSnapshot({
          ...
          activeDocumentPath: path,
          activeDocumentSource:
            path === alphaPath ? "# Alpha cached" : "# Beta cached",
          mode: "Editor",
        }),
      ),
    );
      ...
      ({ path }: { path: string }) =>
        Promise.resolve(
          baseSnapshot({
            ...
            activeDocumentPath: path,
            activeDocumentSource:
              path === alphaPath ? "# Alpha latest" : "# Beta latest",
            mode: "Editor",
          }),
        ),
    );
  ...
  it("continues blocking Save after Keep local until the conflict is resolved", async () => {
    ...
    fireEvent.click(
      within(firstMenu).getByRole("menuitem", { name: /^save$/i }),
    );
  ...
  it("lets the user compare local and disk versions when external changes are detected", async () => {
    hasActiveDocumentExternalChangesMock.mockResolvedValue(true);
    activeDocumentDiskSourceMock.mockResolvedValue(
      "# Meeting notes\n\nUpdated from disk",
    );
    bootstrapMock.mockResolvedValue(
      baseSnapshot({
  ...
  it("does not show a stale disk comparison after switching tabs during Compare", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.queryByText(/External change detected/i),
      ).not.toBeInTheDocument();
    });
  ...
  it("preserves the unsaved draft when opening a workspace", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole("textbox", { name: /source editor/i }),
  ...
  it("keeps the latest dropped document when an earlier drop resolves later", async () => {
    ...
    const alphaDrop = emitDrop({
      payload: { type: "drop", paths: [alphaPath] },
    });
    ...
    expect(sourceEditor).toHaveValue("# Beta");
    expect(
      screen.queryByRole("tab", { name: /alpha\.md/i }),
    ).not.toBeInTheDocument();
    expect(screen.getByRole("tab", { name: /beta\.md/i })).toHaveAttribute(
      "aria-selected",
      "true",
  ...
  it("renders the selected mode before backend mode persistence resolves", async () => {
    ...
    await screen.findAllByText(/^meeting-notes\.md/);
    expect(
      screen.queryByRole("textbox", { name: /source editor/i }),
    ).not.toBeInTheDocument();
  ...
  it("keeps the latest mode selected when earlier mode persistence resolves later", async () => {
    const pendingModeResolutions = new Map<
      EditorMode,
      (snapshot: AppSnapshot) => void
    >();
    ...
    expect(
      await screen.findByTestId("editor-surface-preview"),
    ).toBeInTheDocument();
  ...
  it("switches modes even when dirty draft sync fails", async () => {
    ...
    replaceActiveDocumentSourceMock.mockRejectedValueOnce(
  ...
  it("opens the Document Stats dialog from the Command Palette", async () => {
    ...
    expect(
      await screen.findByRole("dialog", { name: /document stats/i }),
    ).toBeInTheDocument();
  });
  ...
  it("opens in-document Find with Cmd+F and selects the first source match", async () => {
    ...
    const findInput = within(search).getByRole("textbox", {
      name: /find text/i,
    });

    fireEvent.change(findInput, { target: { value: "alpha" } });
    expect(
      within(search).getByText(/press enter to search/i),
    ).toBeInTheDocument();
    ...
    fireEvent.click(
      within(search).getByRole("button", { name: /next match/i }),
    );
  ...
  it("replaces all regex source matches from the replace bar", async () => {
    ...
    fireEvent.click(
      within(search).getByRole("button", { name: /use regular expression/i }),
    );
    ...
    fireEvent.click(
      within(search).getByRole("button", { name: /^replace all$/i }),
    );
  ...
  it("does not push editor-authored WYSIWYG updates back into Tiptap during IME composition", async () => {
    const editor = createMockTiptapEditor("# 안녕하세요", [
      { text: "안녕하세요", from: 1 },
    ]);
    tiptapMockState.editor = editor;
    bootstrapMock.mockResolvedValue(
  ...
  it("defers WYSIWYG CJK draft sync until IME composition ends", async () => {
    ...
    act(() => {
      const handler =
        tiptapMockState.lastOptions.editorProps.handleDOMEvents.compositionend;
  ...
  it("preserves WYSIWYG content when paste arrives while activeDocumentSource is still null", async () => {
    ...
    const editor = createMockTiptapEditor("Existing", [
      { text: "Existing", from: 1 },
    ]);
    tiptapMockState.editor = editor;
    bootstrapMock.mockResolvedValue(
      baseSnapshot({
  ...
  it("inspects an ordinary WYSIWYG markdown link without navigating", async () => {
    const editor = createMockTiptapEditor("[Next](./next.md)", [
      { text: "Next", from: 1 },
    ]);
    tiptapMockState.editor = editor;
    bootstrapMock.mockResolvedValue(
  ...
  it("opens a Cmd-clicked WYSIWYG markdown link in a new tab", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByRole("tab", { name: /current\.md/i }),
      ).toBeInTheDocument();
  ...
  it("swallows the WebKit Korean IME duplicate-syllable handleTextInput call", async () => {
    ...
    const handleTextInput = tiptapMockState.lastOptions.editorProps
      .handleTextInput as (
      view: unknown,
      from: number,
      to: number,
    ...
    const compositionstart = tiptapMockState.lastOptions.editorProps
      .handleDOMEvents.compositionstart as (
      view: unknown,
      event: Event,
    ) => boolean;
    const compositionend = tiptapMockState.lastOptions.editorProps
      .handleDOMEvents.compositionend as (
      view: unknown,
      event: Event,
    ) => boolean;
  ...
  it.each([
    ...
    async (_label, selection, modifier) => {
      const editor = createMockTiptapEditor("docs", [
        { text: "docs", from: 1 },
      ]);
      tiptapMockState.editor = editor;
      bootstrapMock.mockResolvedValue(
  ...
  it("moves WYSIWYG Home and End to the current visual line boundaries", async () => {
    ...
    expect(
      tiptapMockState.lastOptions.editorProps.handleKeyDown(
        editor.view,
        homeEvent,
      ),
    ).toBe(true);
    expect(editor.view.posAtCoords).toHaveBeenLastCalledWith({
      left: 11,
      top: 110,
    });
  ...
  it("replaces all WYSIWYG RTL matches through a ProseMirror text transaction", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("mock-tiptap-editor")).toHaveAttribute(
        "data-selection-from",
        "1",
      );
      expect(within(search).getByText("No matches")).toBeInTheDocument();
  ...
  it("does not open Find from an IME-composing command shortcut", async () => {
    ...
    expect(
      screen.queryByRole("search", { name: /find and replace/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders friendly mode labels in the app menu and status bar", async () => {
    ...
    expect(statusBar).not.toBeNull();
    expect(
      within(statusBar as HTMLElement).getByText("Split View"),
    ).toBeInTheDocument();
  });
  ...
  it("resets the sidebar width to the default when the resize separator is double-clicked", async () => {
    ...
    try {
      ...
      expect(window.localStorage.getItem("markdowner.sidebarWidth")).toBe(
        "280",
      );
    } finally {
      window.localStorage.removeItem("markdowner.sidebarOpen");
  ...
  it("groups Quick Open entries under Workspace Files and Recent Files section headers", async () => {
    bootstrapMock.mockResolvedValue(
      baseSnapshot({
        ...
        recentDocuments: ["/tmp/project/README.md", "/tmp/elsewhere/notes.md"],
      }),
    );
  ...
  it("keeps the latest clicked search result active when an earlier open resolves later", async () => {
    ...
    const matches = await within(searchPanel).findAllByTestId(
  ...
  it("keeps search results empty when a pending search resolves after clearing the query", async () => {
    ...
    searchWorkspaceMock.mockImplementation(
      () =>
        new Promise<{ files: ReturnType<typeof workspaceSearchFile>[] }>(
          (resolve) => {
    ...
    await waitFor(() => {
      expect(
        within(searchPanel).queryByText("Searching…"),
      ).not.toBeInTheDocument();
      expect(
        within(searchPanel).getByText(/type to search workspace/i),
      ).toBeInTheDocument();
    });
    ...
    expect(
      within(searchPanel).queryByText(/1 result in 1 file/i),
    ).not.toBeInTheDocument();
  ...
  it("keeps the shell busy while any overlapping document open is still pending", async () => {
    ...
    expect(
      screen.getByRole("status", { name: /working/i }),
    ).toBeInTheDocument();
  ...
  it("toggles the sidebar with Cmd+Shift+B", async () => {
    ...
    await waitFor(() => {
      ...
      expect(window.localStorage.getItem("markdowner.sidebarOpen")).toBe(
        "true",
      );
    });
  ...
  it("reserves a draggable top titlebar with a compact app menu", async () => {
    ...
    const menuButton = within(titlebar).getByRole("button", {
      name: /^app menu$/i,
    });
  ...
  it("toggles AI Feature with Cmd+Shift+A and documents the shortcut", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "AI Feature (Cmd+Shift+A)" }),
    ).toHaveAttribute("aria-keyshortcuts", "Meta+Shift+A Control+Shift+A");
  ...
  it("leaves Cmd+B available for bold instead of toggling the sidebar", async () => {
    ...
    const explorerButton = await screen.findByRole("button", {
      name: /^explorer/i,
    });
  ...
  it("groups Command Palette entries contiguously in File → View → Terminal → Preferences → Theme order", async () => {
    ...
    const categories = options.map(
      (option) => option.getAttribute("data-category")?.trim() ?? "",
    );
    ...
    const headers = within(dialog).getAllByText(
      /^File$|^View$|^Terminal$|^Preferences$|^Theme$/,
    );
    ...
    expect(headerCategories).toEqual([
      "File",
      "View",
      "Terminal",
      "Preferences",
  ...
  it('renders the Command Palette empty-state placeholder with role="presentation" when no commands match', async () => {
    ...
    const placeholder = dialog.querySelector(
  ...
  it("jumps to the last and first Quick Open option with End and Home keys", async () => {
    ...
    await waitFor(() => {
      ...
      expect(refreshed[refreshed.length - 1]).toHaveAttribute(
        "data-active",
        "true",
      );
    });
  ...
  it("keeps the Settings panel layout usable at compact window widths", async () => {
    ...
    const fontFamilyRow = within(panel).getByTestId(
      "settings-field-font-family",
    );
    const fontFamilyInput = within(panel).getByLabelText(/^font family$/i);
    const defaultModeToggle = within(panel).getByTestId(
      "settings-default-mode-toggle",
    );
      ...
      expect(
        tiptapMockState.lastOptions.extensions.some(
          (extension: { name?: string }) =>
            extension?.name === "wysiwygSkillHighlight",
        ),
      ).toBe(true);
    });
  }, 10_000);
  ...
  it("persists WYSIWYG Code Block Wrap changes from the Settings dialog through save_settings", async () => {
    ...
    const dialog = await screen.findByTestId("settings-panel");
    const wrapToggle = within(dialog).getByLabelText(
      /WYSIWYG Code Block Wrap/i,
    );
  ...
  it("persists inline style colors and applies the active theme palette immediately", async () => {
    ...
    fireEvent.keyDown(window, { key: ",", metaKey: true });
    const dialog = await screen.findByTestId("settings-panel");
    fireEvent.change(within(dialog).getByLabelText("Skill Token Dark Text"), {
      target: { value: "#123456" },
    });
  ...
  it("persists Word Break Keep All changes from the Settings dialog through save_settings", async () => {
    ...
    const dialog = await screen.findByTestId("settings-panel");
    const wordBreakToggle =
      within(dialog).getByLabelText(/word break keep all/i);
  ...
  it("persists Default Startup Mode changes from the Settings dialog through save_settings", async () => {
    ...
    const dialog = await screen.findByTestId("settings-panel");
    const splitViewToggle = within(dialog).getByRole("radio", {
      name: /split view/i,
    });
  ...
  it("persists Typewriter Mode changes from the Settings dialog through save_settings", async () => {
    ...
    const dialog = await screen.findByTestId("settings-panel");
    const typewriterModeToggle =
      within(dialog).getByLabelText(/typewriter mode/i);
  ...
  it("persists Outline density changes from the Settings dialog through save_settings", async () => {
    ...
    const rowSpacingInput =
      within(dialog).getByLabelText(/outline row spacing/i);
  ...
  it("persists Diagnostics logging changes from the Settings dialog through save_settings", async () => {
    ...
    const diagnosticsToggle =
      within(dialog).getByLabelText(/diagnostics logging/i);
    await waitFor(() => {
  ...
  it("persists Usage analytics opt-out from the Settings dialog through save_settings", async () => {
    ...
    const dialog = await screen.findByTestId("settings-panel");
    const analyticsToggle = within(dialog).getByLabelText(
      /share anonymous usage data/i,
    );
    await waitFor(() => {
  ...
  it("shows the update banner when a manual update check is already on the latest version", async () => {
    ...
    expect(
      within(banner).getByText(/already on the latest version/i),
    ).toBeInTheDocument();
  });
  ...
  it("runs a manual update check from the Command Palette", async () => {
    ...
    fireEvent.click(
      within(dialog).getByRole("option", { name: /check for updates/i }),
    );
  ...
  it("downloads and installs the latest update from the Command Palette", async () => {
    ...
    const checkDialog = await screen.findByRole("dialog", {
      name: /command palette/i,
    });
    const checkInput = within(checkDialog).getByRole("textbox", {
      name: /command palette search/i,
    });
    fireEvent.change(checkInput, { target: { value: "check updates" } });
    fireEvent.click(
      within(checkDialog).getByRole("option", { name: /check for updates/i }),
    );
    ...
    const installInput = within(installDialog).getByRole("textbox", {
      name: /command palette search/i,
    });
  ...
  it("opens multiple Markdown documents as tabs when the dialog returns an array", async () => {
    ...
    expect(openDocumentMock).toHaveBeenNthCalledWith(
      1,
      "/tmp/project/alpha.md",
    );
  ...
  it("collapses a carried source selection when opening a new document", async () => {
    ...
    await waitFor(() => {
      ...
      ).not.toBe(`0:${sourceTextarea.value.length}`);
    });
  });

  it("collapses WYSIWYG editor state selection when opening a different document", async () => {
    const editor = createMockTiptapEditor("0123456789", [
      { text: "0123456789", from: 1 },
    ]);
  ...
  it("opens a recent document from the native menu event", async () => {
    ...
    await menuCommandHandler?.({
      payload: "open-recent-document:/tmp/project/meeting-notes.md",
    });
  ...
  it("activates a document opened while the Settings tab is active", async () => {
    ...
    await menuCommandHandler?.({
      payload: "open-recent-document:/tmp/project/next.md",
    });
  ...
  it("applies native update-snapshot events from external file opens", async () => {
    ...
    expect(
      screen.getByRole("tab", { name: /launched\.md/i }),
    ).toBeInTheDocument();
  });
  ...
  it("keeps a CLI-launched Markdown file active instead of replacing it with restored tabs", async () => {
    ...
    try {
      ...
      expect(
        screen.queryByRole("tab", { name: /previous\.md/i }),
      ).not.toBeInTheDocument();
      expect(openDocumentMock).not.toHaveBeenCalledWith(
        "/tmp/project/previous.md",
      );
      await runtimeErrors.expectClean();
    } finally {
      runtimeErrors.restore();
  ...
  it("restores and persists terminal height through the terminal resize handle", async () => {
    ...
    try {
      ...
      expect(window.localStorage.getItem("markdowner.terminalHeight")).toBe(
        "368",
      );
    } finally {
